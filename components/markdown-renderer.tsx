...
export default function MarkdownRenderer({ content }: MarkdownRendererProps) {
  // Extract chart data from JSON code blocks
              ...
              components={{
                ...
                code: ({ node, inline, ...props }) => {
                  if (inline) {
                    return (
                      <code
                        className="bg-zinc-800 text-emerald-400 px-2 py-1 rounded font-mono text-sm"
                        {...props}
                      />
                    )
                  }
                  return (
                    <code
                      className="block bg-zinc-800 text-zinc-100 p-4 rounded-lg font-mono text-sm overflow-x-auto mb-3"
                      {...props}
                    />
                  )
                },
                pre: ({ node, ...props }) => (
                  <pre className="bg-zinc-800 text-zinc-100 p-4 rounded-lg overflow-x-auto mb-3" {...props} />
                ),
                // Links
                a: ({ node, ...props }) => (
                  <a
                ...
                blockquote: ({ node, ...props }) => (
                  <blockquote
                    className="border-l-4 border-emerald-500 pl-4 py-2 text-zinc-300 italic mb-3"
                    {...props}
                  />
                ),
                // Tables
